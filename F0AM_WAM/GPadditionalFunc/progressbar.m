function [stopBar] =  progressbar(fractiondone, position,usrTitle)
...
try %if previous progressbar is closed but handles are still there, error will result, this clears all persistent variables from memory and tryes again
    ...
    if (fractiondone == 0)
        ...
        titlebarstr = sprintf('%2d%%    %s remaining %s',percentdone,timeleftstr,usrTitle);
    end
    ...
    clear progfig progpatch starttime lastupdate firstIteration
    [stopBar] = progressbar(fractiondone, position,usrTitle);
end
%%
% ------------------------------------------------------------------------------
