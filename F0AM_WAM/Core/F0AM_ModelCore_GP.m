function S = F0AM_ModelCore_GP(Met,InitConc,seedValues,ChemFiles,BkgdConc,ModelOptions,SolarParam)
...
FieldInfo = {...
    ...
    'kwall_loss'    0               0;...
    'kwall_lossV'   0               0;...
    'Cstar_threshold' 1             100;...
    'EF_C5H8'       0               0;...  %added these for PNNL chamber 7/20/16 ELD
    'EF_H2O2'       0               0;...
    'EF_NO'         0               0;...
    };
...
[Cnames,Rnames,k,f,iG,iRO2,Met.jcorr,Met.jcorr_all,iOA,iVapor] = InitializeChemistry_GP(Met,ChemFiles,ModelOptions,1);

%write new m file
[particle,Cstar] = F0AM_GP_ChemistryGenerator(Met.Cstar_threshold,Met.P(1)./1.333,Met.T(1),Cnames); %ug/m3, torr, K
%add new m file to chem files
ChemFiles{end+1} = 'GPpartitioning';
%add to chem files
[Cnames,Rnames,k,f,iG,iRO2,Met.jcorr,Met.jcorr_all,iOA,iVapor] = InitializeChemistry_GP(Met,ChemFiles,ModelOptions,0);
...
% get indices of particle phase reactions, i.e. reactions that involve
% addition to an OA or evaporation
icondensat = strfind(Rnames, '+ OA'); ievaporat = strfind(Rnames, '+ Evaporation'); 
for a = 1:nRx; 
    icondensation(a)=logical(~isempty(icondensat{a})); 
    ievaporation(a)=logical(~isempty(ievaporat{a})); 
end

Cstar = k(:,ievaporation); %molec/cm3
...
if conc_init(:,3)~=0
    conc_init(:,4)= 1./conc_init(:,3);
else
    conc_init(:,4) = 2e-8; %if there's no iOA, 1./sum will be infinity and mess up concentration calculations
end
...
%                 NOxinfo = [iNOx;initNOx]; %for adjustment in dydt_eval
        ...
        if SolarFlag
            ...
            for h = 1:nSolar
                ...
                %%%%% CALL ODE SOLVER %%%%%
                for h = 1:ModelOptions.GPupdateTime:ModelOptions.IntTime; %added this 2/9/16
            
                MWoa = nansum(particle.mass.*conc_init_step(iOA,:))./nansum(conc_init_step(iOA,:)); %g/mol
                if isnan(MWoa); MWoa = 0; end
                densityOA = 1.2; %g/cm3. guess!!
                VOA = (conc_init_step(3,:)./6.02E23).*(MWoa./densityOA); %Vseed cm3/cm3
                Vtotal = VOA + seedValues{3,2};
                Rp = ((3.*Vtotal)./(4.*pi.*seedValues{4,2})).^(1/3); %Rp cm
                SA = (3.*Vtotal)./Rp; %SA cm2/cm3
                mass = repmat(particle.mass,1,size(k,1));
                k(:,icondensation) = k_mass_transfer_A(Rp, 0.1, 0.1, mean_speed(mass', 298)).*SA;
                k(:,ievaporation) = k_mass_transfer_A(Rp, 0.1, 0.1, mean_speed(mass', 298)).*SA.*Cstar;
                    ...
                    [time_out,conc_out] = ode15s(@(t,conc_out) dydt_eval_GP(t,conc_out,param),...
                        [0 ModelOptions.GPupdateTime],conc_init_step',options);
    ...
    %                     NOxinfo = [iNOx;initNOx]; %for adjustment in dydt_eval
                    ...
                    progressbar(h./ModelOptions.IntTime); 
                end
            ...
            for h = 1:ModelOptions.GPupdateTime:ModelOptions.IntTime; %added this 2/9/16
                ...
                Rp = ((3.*Vtotal)./(4.*pi.*seedValues{4,2})).^(1/3); %Rp cm
                SA = (3.*Vtotal)./Rp; %SA cm2/cm3
                mass = repmat(particle.mass,1,size(k,1)); %if there's multiple runs you need a row for each one and particle.mass is just a vector
                k(:,icondensation) = k_mass_transfer_A(Rp,0.1,0.1,mean_speed(mass',298)).*SA;
                k(:,ievaporation) = k_mass_transfer_A(Rp,0.1,0.1,mean_speed(mass',298)).*SA.*Cstar;
                ...
                conc_init_step = conc_out(end,:)';
                conc_run = [conc_run, conc_out(end,:)'];
                t_out = [t_out; h];
        ...
        elseif ~SolarFlag
            Conc = [Conc; conc_run'];
            Time = [Time; t_out];
            StepIndex = [StepIndex; i*ones(size(t_out))];
            RepIndex = [RepIndex; j*ones(size(t_out))];
        else
...
if ~isempty(ModelOptions.TimeStamp)
    ...
    dTime = nanmedian(diff(Time2Rep)); %assumes equal spacing
...
clear G kbig Mbig kdilbig conc_bkgd_big kdilbig conc_bkgd_big icondensation
clear conc_init conc_bkgd dilrate kdil M Time2Rep dTime conc_run 
clear nRx nSp nIc nSolar a ans conc_run iVapor icondensat ievaporat t_out
clear f iG k ChemFiles Rnames iHold Mnames ievaporation mass seedValues
